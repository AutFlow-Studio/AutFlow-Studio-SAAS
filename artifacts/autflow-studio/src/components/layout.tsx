    ...
    <div className="flex h-screen w-full bg-background overflow-hidden selection:bg-primary/30">
      ...
      <aside className={cn(
        ...
        <div className="p-4 border-t border-border/50 flex flex-col gap-1">
          {user?.role === "owner" && (
            <Link href="/integrations" onClick={() => setMobileNavOpen(false)} className={cn(
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="h-16 flex items-center justify-between px-4 md:px-6 border-b border-border/50 bg-background/80 backdrop-blur-md z-10 sticky top-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="w-8 h-8 rounded-full overflow-hidden ring-2 ring-background cursor-pointer flex-shrink-0 shadow-sm">
                  {user?.avatarUrl ? (
                    <img
                      src={`/api/storage${user.avatarUrl}`}
                      alt={user.name}
