...
export async function sendVerificationEmail({
  ...
  if (!client) {
    console.warn("[mailer] RESEND_API_KEY is not set — verification email not sent.");
    console.warn(`[mailer] Verify URL for ${to}: ${verifyUrl}`);
    return;
  }
  ...
  const html = emailShell(`
    <tr>
      <td style="padding-bottom:24px;">
        <h1 style="margin:0 0 12px;font-size:22px;font-weight:600;color:#fafafa;">Verify your email</h1>
        <p style="margin:0 0 8px;font-size:15px;color:#a1a1aa;line-height:1.6;">Hi ${escapeHtml(name)},</p>
        <p style="margin:0 0 24px;font-size:15px;color:#a1a1aa;line-height:1.6;">
          Thanks for signing up! Click the button below to verify your email address and activate your workspace.
          This link expires in <strong style="color:#d4d4d8;">24 hours</strong>.
        </p>
    ...
    <tr>
      <td style="padding-top:24px;border-top:1px solid #27272a;">
        ...
        <p style="margin:0;font-size:12px;color:#7c3aed;word-break:break-all;">${verifyUrl}</p>
      </td>
    </tr>
    <tr>
      <td style="padding-top:24px;">
        <p style="margin:0;font-size:13px;color:#52525b;line-height:1.5;">
          If you didn't create an AutFlow Studio account, you can safely ignore this email.
        </p>
      </td>
    </tr>
  `);
  ...
  await client.emails.send({
    from: FROM_EMAIL,
    to,
    subject: "Verify your AutFlow Studio email address",
    html,
  });
}
