...
router.get("/notifications", async (req, res): Promise<void> => {
  ...
  const notifications = await db
    ...
    .where(eq(notificationsTable.workspaceId, wid))
...
router.get("/notifications/unread-count", async (req, res): Promise<void> => {
  ...
  const [row] = await db
    ...
    .from(notificationsTable)
    .where(and(eq(notificationsTable.isRead, false), eq(notificationsTable.workspaceId, wid)));
...
router.patch("/notifications/:id/read", async (req, res): Promise<void> => {
  ...
  const [notification] = await db
    ...
    .set({ isRead: true })
    .where(and(eq(notificationsTable.id, id), eq(notificationsTable.workspaceId, wid)))
    .returning();
